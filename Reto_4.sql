...
SELECT Year, Day, Carrito, Ingrediente, aperturas+cierres+ventas+refills AS Diff
FROM (
SELECT YEAR(posttime) AS Year, DAY(posttime) AS Day,
c.placa AS Carrito,
i.descripcion AS Ingrediente,
SUM(CASE WHEN il.operationType = 2 THEN il.quantity ELSE 0 END) AS aperturas,
SUM(CASE WHEN il.operationType = 3 THEN il.quantity ELSE 0 END) AS cierres,
SUM(CASE WHEN il.operationType = 0 THEN il.quantity ELSE 0 END) AS ventas,
SUM(CASE WHEN il.operationType = 1 THEN il.quantity ELSE 0 END) AS  refills
FROM inventorylogs il
INNER JOIN ingredientes i ON il.ingredientesID = i.ingredienteID
INNER JOIN carritos c ON il.carritoID = c.carritoID
