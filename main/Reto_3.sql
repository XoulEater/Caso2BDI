...
SELECT 
    YEAR(v.posttime) AS Year,
    WEEK(v.posttime) AS Week,
    c.nombre AS Copero,
...
FROM
    ventas v
        INNER JOIN
    coperos c ON v.coperoID = c.coperoID
...
FROM
    ingredientes i
        INNER JOIN
    inventorylogs il ON il.ingredientesID = i.ingredienteID
        AND il.operationType = 0
